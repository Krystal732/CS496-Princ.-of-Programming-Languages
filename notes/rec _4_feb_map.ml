...
let rec succl : int list -> int list =
  ...
  | [] -> []
  | h::t -> inc h :: succl t
(*turn chars to all uppercase *)
let rec upperl : char list -> char list =
  fun l ->
  match l with
  | [] -> []
  | h::t -> upper h :: upperl t
(*replaces elemets with false or true if it is a 0*)
...
(*same functions as before but now using map*)
...
(*and of all elements of list*)
 let rec andl : bool list -> bool =
    ...
    | [] -> true
    | h::t -> h && andl t
(*list of lists and concat all them into one list*)                
